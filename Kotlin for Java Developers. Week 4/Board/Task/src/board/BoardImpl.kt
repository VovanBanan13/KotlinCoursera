...
open class SquareBoardImpl(override val width: Int): SquareBoard {
    ...
    init {
        (1..width).forEach { i -> var row = arrayOf<Cell>()
            (1..this.width).forEach { j -> row += Cell(i, j)}
        cells += row
        }
    }
    ...
    override fun getAllCells(): Collection<Cell> {
        return IntRange(1, width).flatMap { i: Int -> IntRange(1, width).map { j: Int -> getCell(i, j) } }.toList()
    }
